...
parser.add_argument(
    "--vrephoto_dir",
    type=str,
    help="Path to the output directory for .cam and .res files",
)
parser.add_argument(
    "--out_colmap_model",
    type=str,
    help="Path to the directory where will be created the COLMAP model",
)
...
parser.add_argument(
    ...
    nargs="+",
    default=[0, 0, 1],
    help="Up vector (Z+) in the mesh coordinate frame (vector of length 3)",
)
parser.add_argument(
    "--pos_sampling_num",
    type=int,
    default=1200,
    help="Number of initial camera position samples (in the mesh axis-aligned bounding box)",
)
parser.add_argument(
    "--pos_sampling_mode",
    type=str,
    ...
    help="Camera positions sampling method (regular/random).",
)
parser.add_argument(
    "--bbox_margin_limits",
    type=float,
...
parser.add_argument(
    ...
    type=int,
    default=3,
    help="Number of initial orientaion samples per single camera position sample",
)
parser.add_argument(
    "--ang_sampling_mode",
    type=str,
    default="regular_rand_yaw_pitch",
    choices=[
        "regular",
        "regular_rand_yaw",
        "regular_rand_yaw_pitch",
        "random_yaw",
        "random_yaw_pitch",
    ],
    help="Camera orientation sampling method.",
)
parser.add_argument(
    "--ang_yaw_std",
    type=float,
    default=15,
    help="Yaw standard deviation (if using random yaw) [degrees]",
)
parser.add_argument(
    "--ang_pitch_std",
    type=float,
    default=15,
    help="Pitch standard deviation (if using random pitch) [degrees]",
)
...
parser.add_argument(
    ...
    type=bool,
    default=True,
    help="Check if the casted ray intersects the mesh surface from the front or from the back side",
)
...
def main(args):
    ...
    if args.pos_sampling_mode == "regular":
        ...
        samples_x = mesh_center[0] + np.linspace(
            -0.5 * (sample_num_axes[0] - 1) * sample_spacing,
            0.5 * (sample_num_axes[0] - 1) * sample_spacing,
            sample_num_axes[0],
        )
        samples_y = mesh_center[1] + np.linspace(
            -0.5 * (sample_num_axes[1] - 1) * sample_spacing,
            0.5 * (sample_num_axes[1] - 1) * sample_spacing,
            sample_num_axes[1],
        )
        samples_z = mesh_center[2] + np.linspace(
            -0.5 * (sample_num_axes[2] - 1) * sample_spacing,
            0.5 * (sample_num_axes[2] - 1) * sample_spacing,
            sample_num_axes[2],
        )
        ...
        samples_x = np.random.uniform(
            low=mesh_min_bounds[0],
            high=mesh_max_bounds[0],
            size=(args.pos_sampling_num),
        )
        samples_y = np.random.uniform(
            low=mesh_min_bounds[1],
            high=mesh_max_bounds[1],
            size=(args.pos_sampling_num),
        )
        samples_z = np.random.uniform(
            low=mesh_min_bounds[2],
            high=mesh_max_bounds[2],
            size=(args.pos_sampling_num),
        )
    ...
    xy_valid = np.logical_and(
        xy_ray_lenghts >= args.xy_limits[0], xy_ray_lenghts <= args.xy_limits[1]
    )
    ...
    if args.ang_sampling_mode in [
        ...
        if args.ang_sampling_mode in ["regular_rand_yaw", "regular_rand_yaw_pitch"]:
            yaw_samples = yaw_samples + np.random.normal(
                loc=0.0, scale=args.ang_yaw_std, size=(yaw_samples.shape[0])
            )

            if args.ang_sampling_mode in ["regular_rand_yaw_pitch"]:
                pitch_samples = pitch_samples + np.random.normal(
                    loc=0.0, scale=args.ang_pitch_std, size=(pitch_samples.shape[0])
                )

    elif args.ang_sampling_mode in ["random_yaw", "random_yaw_pitch"]:
        yaw_samples = np.random.uniform(
            low=0, high=360, size=(args.ang_sampling_num * pos_samples.shape[1])
        )
        pitch_samples = np.full_like(yaw_samples, -90)
    ...
    if args.vrephoto_dir is not None:
        ...
        write_vrephoto(
            args.vrephoto_dir, cams_T, args.cam_foclen, args.img_width, args.img_height
        )
    ...
    if args.vis_mode != "off":
        ...
        color_vals = (mesh_verts_z - mesh_verts_z.min()) / (
            mesh_verts_z.max() - mesh_verts_z.min()
        )
