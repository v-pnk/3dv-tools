...
parser.add_argument("out_colmap_model", type=str,
    help="Path to the directory for the output COLMAP model")
parser.add_argument("--rendering_suffix", type=str, default="_rendered_color.png",
    help="Suffix of rendered images (for COLMAP model - default: %(default)s)")
parser.add_argument("--colmap_database", type=str,
    help="Path to COLMAP database (is used to get image IDs)")
parser.add_argument("--render_config_file", type=str,
    help="YAML config with the radius, azimuth and elevation definitions")
parser.add_argument("--azimuth_samples", type=int, default = 36,
    help="Number of samples in the azimuth axis (on 360 degs) - default: %(default)s")
parser.add_argument("--elevation_samples", type=int, default = 8, 
    help="Number of samples in the elevation (on 180 degs) - default: %(default)s")
parser.add_argument("--radius_mult", type=float, nargs='+', default = [3.0], \
    help="Multiplier of the radius estimated from the bounding box of the given mesh - default: %(default)s")
parser.add_argument("--vert_axis", type=float, nargs='+', default=[0.0,0.0,1.0],
    help="Vertical axis  - default: %(default)s")
parser.add_argument("--center_shift", type=float, nargs='+', default=[0.0,0.0,0.0],
...
parser.add_argument("--cam_foclen", type=float, default = 800,
    help="Image height - default: %(default)s")
parser.add_argument("--elevation_lims", type=float, nargs='+', default=[-10.0, 60.0], \
    help="Limits of the elevation angle (degrees) - default [0, 90] (= upper halfsphere) - default: %(default)s")
parser.add_argument("--azimuth_lims", type=float, nargs='+', default=[0, 360.0], \
    help="Limits of the elevation angle (degrees) - default [0, 90] (= upper halfsphere) - default: %(default)s")
parser.add_argument('--dont_visualize', action="store_true",
    help='Turns off the visualization of camera poses.')
parser.add_argument("--rel_cam_scale", default=0.05, type=float,
    help="Scale of the camera 3D model relative to the length of longest side of the bounding box of the mesh model")
parser.add_argument("--vis_cam_color", default=[66/255, 159/255, 255/255], type=float, nargs="+",
    help="Color of the visualized camera poses - three values (R,G,B) in [0,1] range")
parser.add_argument('--photo_mode', action="store_true",
    help='Do not show coordinate frames, only the mesh and camera frustums.')
parser.add_argument("--mesh_color_mode", type=str, default="z", choices=["z", "shaded"], 
    help="Coloring mode of the mesh - default: %(default)s")
parser.add_argument("--mesh_color", type=float, default=[66/255, 159/255, 255/255], nargs="+", 
...
def main(args):
    assert os.path.exists(args.mesh_path), "The given mesh file does not exist: {}".format(args.mesh_path)

    if args.render_config_file is not None:
        load_render_config(args.render_config_file, args)

    assert len(args.vert_axis) == 3, "The given vertical axis must have 3 dimensions, but has {}.".format(len(args.vert_axis))
    assert len(args.center_shift) == 3, "The given center shift must have 3 dimensions, but has {}.".format(len(args.center_shift))

    assert len(args.elevation_lims) == 2, "The elevation limits parameter must be a list of 2 elements"
    assert len(args.azimuth_lims) == 2, "The azimuth limits parameter must be a list of 2 elements"
    assert args.elevation_lims[0] <= args.elevation_lims[1], "The first element of elevation limits marks minimum elevation and the second maximum elevation, therefore (min <= max) has to hold"
    ...
    azimuth_arr = np.linspace(args.azimuth_lims[0], args.azimuth_lims[1], args.azimuth_samples + 1)[:-1]


    elevation_arr = np.linspace(args.elevation_lims[0], args.elevation_lims[1], args.elevation_samples)
    ...
    azimuth_mesh, elevation_mesh, radius_mesh = np.meshgrid(azimuth_arr, elevation_arr, radius)
    ...
    if not(args.dont_visualize):
        K = np.array([[args.cam_foclen, 0.0, 0.5 * args.img_width],[0.0, args.cam_foclen, 0.5 * args.img_height],[0.0, 0.0, 1.0]])
    ...
    if args.out_colmap_model is not None:
        if args.colmap_database is not None:
            ...
            print("WARN: The generated COLMAP model image IDs will probably not align to the IDs in any model generated by COLMAP.")
            print("- Create first a model with a database in COLMAP and provide the database to this script to fix the issue.")
        ...
        f_cameras.write("1 PINHOLE {} {} {} {} {} {}".format(
            args.img_width, args.img_height,
            args.cam_foclen, args.cam_foclen,
    ...
    for az, el, r in zip(azimuth_mesh, elevation_mesh, radius_mesh):
        ...
        img_name = "sphere_r{:.0f}_a{:.0f}_e{:.0f}".format(r, np.degrees(el), np.degrees(az))
        ...
        f_images.write("{} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f} 1 {}\n\n".format(
            img_id, q_colmap[0], q_colmap[1], q_colmap[2], q_colmap[3],
            t_colmap[0], t_colmap[1], t_colmap[2], full_img_name))

        if not(args.dont_visualize):
            if not(args.photo_mode):
                T_CS_cam_scale = np.eye(4)
                T_CS_cam_scale[0:3, 0:3] = np.diag(np.tile(args.rel_cam_scale*bbox_max_side, 3))
                ...
                T_mesh_align_scale = np.array([[3,0,0,0],[0,3,0,0],[0,0,3,0],[0,0,0,1]])
            ...
            cam_vis = o3d.geometry.LineSet.create_camera_visualization(args.img_width, args.img_height, K, T_out, args.rel_cam_scale*bbox_max_side)
    ...
    if not(args.dont_visualize):
        if args.mesh_color_mode == "z":
            mesh_pnts = np.linalg.inv(T_vert_axis[0:3,0:3]) @ np.asarray(mesh.vertices).T
            mesh_z = mesh_pnts[2,:]
            mesh_z = (mesh_z - np.min(mesh_z)) / (np.max(mesh_z) - np.min(mesh_z))
            mesh.vertex_colors = o3d.utility.Vector3dVector(
                np.tile(mesh_z, (3, 1)).T)
...
def vert_axis2R(vect_axis):
    ...
    skew_sym_axis = np.array([[0, -axis[2,0], axis[1,0]],[axis[2,0], 0, -axis[0,0]],[-axis[1,0], axis[0,0], 0]])
