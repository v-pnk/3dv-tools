...
parser.add_argument('input_depth', type=str, help='Path to the input depth map in .npy, .npz, .png or .h5 format')
parser.add_argument('--input_rgb', type=str, required=False, help='Path to input RGB image - can be used to color the point cloud')
parser.add_argument('--focal_length', type=float, help='Camera focal length used for the back-projection to point cloud')

parser.add_argument('--visualize_depth', action="store_true", help='Visualize the depth map as image')
parser.add_argument('--visualize_pc', action="store_true", help='Visualize point cloud created from the depth map')

parser.add_argument('--depth_mode', type=str, choices = ['depth', 'distance'], default='depth', help='Switch between the input depth type - \"depth\" is Z distance from camera center, \"distance\" is Euclidean distance from the camera center')
parser.add_argument('--inverse_depth', action="store_true", help='Inverse inverse the input depth values as 1/d')
parser.add_argument('--reverse_color', action="store_true", help='Reverse the used colormap')
parser.add_argument('--depth_multiplier', type=float, default=1.0, help='Depth multiplier')
parser.add_argument('--nan2zero', action="store_true", help='Turn all NaN values to 0')

parser.add_argument('--palette_min', type=float, help='Minimal values visible in color palette')
parser.add_argument('--palette_max', type=float, help='Maximal values visible in color palette')
parser.add_argument('--palette_hide_out', action="store_true", help='Hide elements outside of set palette limits')

parser.add_argument('--output_image', type=str, help='Path to the output depth image')
...
def main(args):
    ...
    if args.visualize_pc or (args.output_pc is not None):
        ...
        if args.depth_mode == "depth":    
            ...
            xyz = xyz * depth_map[Y, X][:, None] / np.tile(np.sqrt(np.sum(xyz**2, axis=1, keepdims=True)), (1,3))
        ...
        color_raw_np_flat = np.reshape(np.swapaxes(np.asarray(color_raw)/255.0, 0, 2), (3,-1), order='F')
        valid_idx = np.logical_and(depth_raw_np_flat > 0.0,
            np.logical_not(np.isinf(depth_raw_np_flat)))
        color_raw_np_flat = color_raw_np_flat[:, valid_idx]
        pcd.colors = o3d.utility.Vector3dVector(color_raw_np_flat.T)

        # - estimate normals
        pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.03, max_nn=20))
...
def visualize_colormap(reverse_color, min_value, max_value):
    ...
    for val_i in colormap_vals:
        ...
        color_strings.append("\x1b[38;2;{};{};{}m".format(
            col_i[0], col_i[1], col_i[2]) + u"\u2588" + "\033[0m")
