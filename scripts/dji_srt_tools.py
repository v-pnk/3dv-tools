...
def main(args):
    ...
    if args.video_path is not None:
        ...
        frames, frame_idx_list = extract_frames(args.video_path, frame_rate=args.frame_rate, res_ratio=args.res_ratio)
        ...
        with exiftool.ExifToolHelper() as et:
            ...
            for frame, idx in zip(frames, frame_idx_list):
                ...
                if "shutter" in frame_metadata[idx]["metadata"]:
                    exif_tags["EXIF:ShutterSpeedValue"] = sec_frac_to_apex(frame_metadata[idx]["metadata"]["shutter"])
...
def load_dji_srt_file(dji_srt_path):
    ...
    assert dji_srt_path.endswith((".srt", ".SRT")), "The file must have the .srt extension"
    ...
    for i in range(0, len(lines), 6):
        ...
        frame_counter = int(
            frame_counter_str.split(",")[0].split(":")[1])
        ...
        coords_wgs84 = np.c_[coords_wgs84, [float(metadata["latitude"]), 
...
def write_geojson(file_path, coords_wgs84, timestamps):
    ...
    linestring = {
        ...
        "geometry": {
            "type": "LineString",
            "coordinates": [[coords_wgs84[1, i], coords_wgs84[0, i]] for i in range(len(timestamps))],
        },
        "properties": {
            "stroke": "#000000",
            "stroke-width": 1,
