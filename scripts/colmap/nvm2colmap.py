...
parser = argparse.ArgumentParser(description="NVM to COLMAP converter")
parser.add_argument(
    "input_nvm", 
    type=str, 
...
def parse_nvm(path):
    ...
    for line in f:
        ...
        if (state == "header") and line:
            ...
            found_name = os.path.join(
                os.path.dirname(name), os.path.basename(found_path)
            )
            ...
            for i in range(meas_num):
                ...
                track_data.append(
                    {"img_id": img_idx, "pnt2_id": len(cam_data[img_idx]["pnt2_data"])}
                )
                # - add 2D point data and 3D point ID to corresponding images
                cam_data[img_idx]["pnt2_data"].append(
                    {"pnt3_id": len(pnt3_data), "kpt_rel_c": kpt_rel_c}
                )
...
def write_colmap(path, cam_data, pnt3_data):
    ...
    for img_id, data in enumerate(cam_data):
        ...
        for pnt2 in data["pnt2_data"]:
            ...
            pnt2_parts.append("{} {} {}".format(kpt[0], kpt[1], pnt2["pnt3_id"]))
