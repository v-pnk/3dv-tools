...
parser.add_argument("--mode", type=str, choices=["fixed_k", "stop_at_min", "divide_and_merge", "bin_agg_clust"], default="bin_agg_clust",
...
parser.add_argument("--covis_fnc", type=str, choices=["lin", "tanh10", "tanh20", "tanh50", "tanh100"], default="lin",
...
def main(args):
    ...
    if args.covis_matrix_path is not None and os.path.isfile(args.covis_matrix_path):
        ...
        for point3D in tqdm(model_in.points3D.values()):
            track_img_idxs = [img_ids.index(trck_elem.image_id) for trck_elem in point3D.track.elements]
            track_img_pairs = [(a, b) for idx, a in enumerate(track_img_idxs) for b in track_img_idxs[idx + 1:]]
            for a_idx, b_idx in track_img_pairs:
                covis_matrix[a_idx, b_idx] += 1
                covis_matrix[b_idx, a_idx] += 1
        
        if args.covis_matrix_path is not None and not os.path.isfile(args.covis_matrix_path):
    ...
    if args.mode == "fixed_k":
        ...
        sc = SpectralClustering(n_clusters=args.n_clusters, affinity="precomputed", n_jobs=-1)
        ...
        print("  - dividing into submodels with max. {} images each".format(args.max_images))
        ...
        while max_images_curr > args.max_images:
            ...
            for submodel_idx, curr_img_idxs in enumerate(submodel_img_idxs):
                if len(curr_img_idxs) > args.max_images:
                    covis_matrix_curr = covis_matrix[np.ix_(curr_img_idxs, curr_img_idxs)]
                    sc = SpectralClustering(n_clusters=2, affinity="precomputed", n_jobs=-1)
                    labels_curr = sc.fit_predict(covis_matrix_curr)
                    submodel_1 = [curr_img_idxs[i] for i, label in enumerate(labels_curr) if label == 0]
                    submodel_2 = [curr_img_idxs[i] for i, label in enumerate(labels_curr) if label == 1]
                    
                    if args.min_images is not None and args.mode == "stop_at_min":
                        if len(submodel_1) < args.min_images or len(submodel_2) < args.min_images:
                            print("  - {} --> {} + {} - too small --> keeping original".format(len(curr_img_idxs), len(submodel_1), len(submodel_2)))
                            submodel_img_idxs_skip.append(curr_img_idxs)
                            continue

                    submodel_img_idxs_new += [submodel_1, submodel_2]
                    print("  - {} --> {} + {}".format(len(curr_img_idxs), len(submodel_1), len(submodel_2)))
                    max_images_curr = max(max_images_curr, max(len(submodel_1), len(submodel_2)))
                else:
        ...
        while len(img_tree) > args.n_clusters:
            ...
            covis_vect_ab = (covis_matrix_agg[a, :] + covis_matrix_agg[b, :]) / args.agg_clust_divider
            covis_matrix_agg = np.delete(covis_matrix_agg, [a, b], axis=0)
            covis_matrix_agg = np.append(covis_matrix_agg, np.reshape(covis_vect_ab, (1, -1)), axis=0)
            covis_vect_ab = np.append(covis_vect_ab, 0)
            # covis_vect_ab = np.append(covis_vect_ab, covis_ab)
            covis_matrix_agg = np.append(covis_matrix_agg, np.reshape(covis_vect_ab, (-1, 1)), axis=1)
    ...
    if args.overlap_perc > 0 or args.overlap_abs > 0:
        ...
        for submodel_idx, curr_img_idxs in enumerate(submodel_img_idxs):
            ...
            for img_idx in curr_img_idxs:
                ...
                oosm_img_idxs = np.where((covis_matrix[img_idx, :] > 0) & (labels != submodel_idx))[0]
                oosm_img_covis = covis_matrix[img_idx, oosm_img_idxs]

                for oosm_img_idx, oosm_img_covis in zip(oosm_img_idxs, oosm_img_covis):
                    oosm_list.append({"insm_img_idx" : img_idx, "oosm_img_idx" : oosm_img_idx, "oosm_img_covis" : oosm_img_covis})
            ...
            for oosm_elem in oosm_list:
                submodel_img_idxs_overlap[submodel_idx].append(oosm_elem["oosm_img_idx"])
    ...
    for submodel_idx, submodel_img_id_list in enumerate(submodel_img_ids):
        print("  - creating submodel {:0>3d} with {} images".format(submodel_idx, len(submodel_img_id_list)))
