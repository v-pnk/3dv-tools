...
def main(args):
    ...
    assert os.path.isdir(
        args.output_colmap
    ), 'Output COLMAP model directory "{}" does not exist'.format(args.output_colmap)
    ...
    if args.img_name is None:
        ...
        assert (
            img_src is not None
        ), 'Given image name "{}" not found in COLMAP model'.format(args.img_name)
    ...
    for point3D in tqdm(points3D_src):
        track_image_ids = [
            ...
            if trck_elem.image_id != img_src.image_id
        ]
