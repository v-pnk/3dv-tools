...
parser.add_argument("--crop_left", type=int, 
                    help="Crop left side at this column")
parser.add_argument("--crop_right", type=int, 
                    help="Crop right side at this column")
parser.add_argument("--crop_bottom", type=int,
                    help="Crop bottom side at this row")
parser.add_argument("--crop_top", type=int,
                    help="Crop top side at this row")
...
def main(args):
    ...
    colors = (data_crop.T.flatten() - data_crop.min()) / (data_crop.max() - data_crop.min())
    colors_rgb = np.vstack((colors, colors, colors))

    data_crop_pnts = np.vstack((xpc.T.flatten(), ypc.T.flatten(), data_crop.T.flatten()))
    o3d_mesh = o3d.geometry.TriangleMesh()
    o3d_mesh.vertices = o3d.utility.Vector3dVector(data_crop_pnts.T)

    pnt_idx = np.reshape(np.arange(crop_width*crop_height),
                        (crop_height, crop_width)).T
    tri_a = np.vstack((
        pnt_idx[:-1, :-1].T.flatten(), pnt_idx[1:, :-1].T.flatten(), pnt_idx[:-1, 1:].T.flatten()))
    tri_b = np.vstack((
        pnt_idx[1:, 1:].T.flatten(), pnt_idx[:-1, 1:].T.flatten(), pnt_idx[1:, :-1].T.flatten()))
