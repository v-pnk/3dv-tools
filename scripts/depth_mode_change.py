...
parser.add_argument('--output_format', type=str, default="npz",
                    choices=["npy", "npz"], help='output format - npy / npz')
...
def main(args):
    ...
    if args.dataparser_transforms is not None:
        assert os.path.isfile(args.dataparser_transforms), "The dataparser_transforms.json file does not exist: {}".format(args.dataparser_transforms)
    ...
    if os.path.isdir(args.input_depth):
        assert os.path.isdir(args.output_depth), "The output depth directory does not exist: {}".format(args.output_depth)
        input_depth_list = os.listdir(args.input_depth)
        input_depth_list=[os.path.join(args.input_depth, file) for file in input_depth_list if os.path.splitext(file)[1] in (".npy", ".npz", ".gz")]
    elif os.path.isfile(args.input_depth):
        assert os.path.isfile(args.output_depth), "The output depth file does not exist: {}".format(args.output_depth)
        input_depth_list = [args.input_depth]
    else:
        assert os.path.exists(args.input_depth), "The input depth path does not exist: {}".format(args.input_depth)
    
    focal_lengths = {}

    if args.colmap_model is not None:
        assert os.path.isdir(args.colmap_model), "The COLMAP model directory does not exist: {}".format(args.colmap_model)
        import pycolmap
        ...
        if model.num_cameras() == 1:
            ...
            for _, img in model.images.items():
                ...
                if args.colmap_basenames:
                    new_name = os.path.splitext(os.path.basename(orig_name).replace('/', '_').replace(' ', '_'))[0]
                else:
                    new_name = os.path.splitext(orig_name.replace('/', '_').replace(' ', '_'))[0]
            ...
            for file_path in input_depth_list:
                file_name, ext = os.path.splitext(os.path.basename(file_path).replace('/', '_').replace(' ', '_'))
                if ext == ".gz":
        ...
        assert args.focal_length is not None, "Please provide a focal length or COLMAP model"
        for file_path in input_depth_list:
    ...
    for input_depth in input_depth_list:
        ...
        if os.path.isdir(args.input_depth):
            output_depth = os.path.join(args.output_depth, os.path.basename(input_depth))
        else:
