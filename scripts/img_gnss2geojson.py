...
parser.add_argument("image_dir", type=str, help="The directory with images (works recursively)")
parser.add_argument("geojson_file", type=str, help="Path to the GeoJSON file")
parser.add_argument("--color_mode", type=str, choices=["timestamp", "month"], help="Mode of coloring of the points")
parser.add_argument("--filter_month", type=str, help="Filter the images by month (1-12) and only include those in the GeoJSON file. Range can also be specified (e.g. 12-2 for December to February)")
...
def main(args):
    ...
    file_list = [os.path.join(dp, f) for dp, dn, filenames in os.walk(args.image_dir) for f in filenames if os.path.splitext(f)[1].lower() in valid_exts]
    ...
    for meta_i, file_i in zip(metadata, file_list):
        ...
        if "EXIF:DateTimeOriginal" in meta_i.keys():
            dt = datetime.strptime(meta_i["EXIF:DateTimeOriginal"], 
                                         "%Y:%m:%d %H:%M:%S")
    ...
    if args.filter_month is not None:
        ...
        img_metadata_all = {key: img_metadata_all[key] for key in img_metadata_all.keys() if img_metadata_all[key]["datetime"].month in months}
    
    print("- coloring points")
    if args.color_mode == "timestamp":
        # - normalize the timestamps to [0,1] range
        all_timestamps = [img_metadata_all[key]["datetime"].timestamp() for key in img_metadata_all.keys() if "datetime" in img_metadata_all[key].keys()]
        ...
        for img_name in img_metadata_all.keys():
            if "datetime" in img_metadata_all[img_name].keys():
                rel_timestamp = (img_metadata_all[img_name]["datetime"].timestamp() - min(all_timestamps)) / total_timespan
                color = list(matplotlib.cm.get_cmap("rainbow")(rel_timestamp)[:3])
                img_metadata_all[img_name]["color"] = tuple([int(255*c) for c in color])
        ...
        "features": [
            {
                ...
                "properties": {
                    "name": name_i, 
                    "datetime": img_metadata_all[name_i]["datetime"].strftime("%Y-%m-%d %H:%M:%S") if "datetime" in img_metadata_all[name_i].keys() else None,
...
def visualize_colormap(color_list, rep_num=1, space_num=0):
    ...
    for row in range(color_list.shape[1]):
        ...
        col_str = ("\x1b[38;2;{};{};{}m".format(col_i[0], col_i[1], col_i[2])
            + "\u2588"
            + "\033[0m")*rep_num
